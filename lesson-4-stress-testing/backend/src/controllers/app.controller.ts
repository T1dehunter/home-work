...
export class AppController {
    ...
    @Get('/test1')
    async getUserData1() {
        const userData = (await this.getDataUseCase.execute1()) as any;
    }

    @Get('/test')
    async getUserData() {
        const userData = (await this.getDataUseCase.execute()) as any;
        return `
            <html>
                <head>
                    <title>Web Page for log user data</title>
                </head>
                <body>
                    <h1>Web Page for test user data!!!</h1>
